...
function initLayout() {
	var iContentH = $(window).height() - $('#navMenu').height() - $('footer').height();
	$("main .tabsPageContent").height(iContentH-$('.tabsPageHeader').height()-3).find("[layoutH]").layoutH();
	$("#splitBar, #splitBarProxy").height(iContentH);
	$("#taskbar").css({
		top : iContentH + $("#navMenu").height(), width : $(window).width()
	});
}
		...
		return this
				.each(function() {
					...
					ths.each(function(i) {
						...
						targetType : $table.attr("targetType"), rel : $table.attr("rel"),
						asc : $table.attr("asc") || "asc", desc : $table.attr("desc") || "desc"
					});
								...
								if ($th.outerWidth() - offset < 5 ) {
									$th.css("cursor", "col-resize").mousedown(function(event) {
										$(".resizeProxy", $grid).show().css({
											...
											top : $.jTableTool.getTop(th), height : $.jTableTool.getHeight(th, $grid),
											cursor : "col-resize"
										});
										$(".resizeMarker", $grid).show().css({
											left : $.jTableTool.getLeft(th) + 1
													- $(".gridScroller", $grid).scrollLeft(),
											top : $.jTableTool.getTop(th), height : $.jTableTool.getHeight(th, $grid)
										});
										$(".resizeProxy", $grid).jDrag($.extend(options, {
											scop : true, cellMinW : 20, relObj : $(".resizeMarker", $grid)[0],
											move : "horizontal", event : event, stop : function() {
	...
	$.jTableTool = {
		getLeft : function(obj) {
			...
			if (!$.support.leadingWhitespace ) {
				...
				var evtset = {
					offsetX : evt.pageX || evt.screenX, offsetY : evt.pageY || evt.screenY
				};
				var offset = {
					offsetX : evtset.offsetX - objset.left, offsetY : evtset.offsetY - objset.top
				};
				return offset;
			}
					...
					return this.each(function() {
						...
						$this.click(function(event) {
							_lookup = $.extend(_lookup, {
								...
								$target : $this, pk : $this.attr("lookupPk") || "id"
							});
					...
					return this.each(function() {
						...
						function _show(event) {
							...
							_lookup = $.extend(_lookup, {
								...
								$target : $input, pk : $input.attr("lookupPk") || "id"
							});
...
(function($) {
	$.extend($.fn, {
		jPanel : function(options) {
			var op = $.extend({
				header : "panelHeader", headerC : "panelHeaderContent", content : "panelContent", coll : "collapsable",
				exp : "expandable", footer : "panelFooter", footerC : "panelFooterContent"
			}, options);
			return this.each(function() {
				var $panel = $(this);
				...
				var title = $(">h1", $panel).wrap('<div class="' + op.header + '"><div class="' + op.headerC
						+ '"></div></div>');
				if (collapse )
					$("<a href=\"\"></a>").addClass(close ? op.exp : op.coll).insertAfter(title);
				var header = $(">div:first", $panel);
				var footer = $('<div class="' + op.footer + '"><div class="' + op.footerC + '"></div></div>')
						.appendTo($panel);
...
(function($) {
	$.extend({
		History : {
			_hash : new Array(), _cont : undefined, _currentHash : "", _callback : undefined,
