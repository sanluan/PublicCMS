...
@Component
public class SystemMoudleDirective extends BaseDirective {

    @Override
    public void execute(RenderHandler handler) throws IOException, Exception {
        Integer id = handler.getInteger(ID);
        if (null != id) {
            SystemMoudle entity = service.getEntity(id);
            handler.put("object", entity).renderIfNotNull(entity);
        } else {
            Integer[] ids = handler.getIntegerArray("ids");
            if (isNotEmpty(ids)) {
                List<SystemMoudle> entityList = service.getEntitys(ids);
                Map<String, SystemMoudle> map = new HashMap<String, SystemMoudle>();
                for (SystemMoudle entity : entityList) {
