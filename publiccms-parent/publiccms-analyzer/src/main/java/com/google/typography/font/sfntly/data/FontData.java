...
public abstract class FontData {
  ...
  public enum DataSize {
    BYTE(1), CHAR(1), USHORT(2), SHORT(2), UINT24(3), ULONG(4), LONG(4), Fixed(4), FUNIT(4), FWORD(
        2), UFWORD(2), F2DOT14(2), LONGDATETIME(8), Tag(4), GlyphID(2), Offset(2);
  ...
  protected FontData(FontData data, int offset) {
    ...
    this.bound(data.boundOffset + offset,
        data.boundLength == FontData.GROWABLE_SIZE ? FontData.GROWABLE_SIZE
            : data.boundLength - offset);
  }

  /**
   * Sets limits on the size of the FontData. The FontData is then only visible
   ...
   *
   * @param offset
   *          the start of the new bounds which must be within the current size
   *          of the FontData
   * @return true if the bounding range was successful; false otherwise
   */
