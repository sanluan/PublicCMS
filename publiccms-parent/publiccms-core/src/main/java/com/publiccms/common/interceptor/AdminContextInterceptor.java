...
public class AdminContextInterceptor extends WebContextInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
		...
		if (adminContextPath.equals(path)) {
			...
			if (null == user) {
				try {
					redirectLogin(ctxPath, path, request.getQueryString(), request.getHeader("X-Requested-With"),
							response);
					return false;
			...
			if (null == entity || entity.isDisabled() || !entity.isSuperuserAccess() || null == site
					|| site.isDisabled() || site.getId() != entity.getSiteId()) {
				try {
					redirectLogin(ctxPath, path, request.getQueryString(), request.getHeader("X-Requested-With"),
							response);
					return false;
	...
	/**
	 * @param adminContextPath the adminContextPath to set
	 */
