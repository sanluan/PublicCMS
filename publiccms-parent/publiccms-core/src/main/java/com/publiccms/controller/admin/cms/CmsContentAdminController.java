...
public class CmsContentAdminController extends AbstractController {
	...
	private String[] ignoreProperties = new String[] { "siteId", "userId", "categoryId", "tagIds", "sort", "createDate",
			"clicks", "comments", "scores", "childs", "checkUserId" };
	 ...
	 */
	@RequestMapping("save")
	public String save(CmsContent entity, CmsContentAttribute attribute,
			@ModelAttribute CmsContentParameters contentParameters, Boolean draft, Boolean checked, String _csrf,
				...
				&& ControllerUtils
						.verifyCustom("noright",
								!(dept.isOwnsAllCategory() || null != sysDeptCategoryService
										.getEntity(new SysDeptCategoryId(user.getDeptId(), entity.getCategoryId()))),
								model)) {
			return CommonConstants.TEMPLATE_ERROR;
		}
		...
		if (null != entity.getId()) {
			...
			entity = service.update(entity.getId(), entity,
					entity.isOnlyUrl() ? ignoreProperties : ignorePropertiesWithUrl);
			if (null != entity) {
		...
		if (entity.isHasImages() || entity.isHasFiles()) {
			contentFileService.update(entity.getId(), user.getId(),
		...
		Map<String, String> map = ExtendUtils.getExtentDataMap(contentParameters.getModelExtendDataList(),
				modelExtendList);
		if (null != category && null != extendService.getEntity(category.getExtendId())) {
			...
			Map<String, String> categoryMap = ExtendUtils
					.getSysExtentDataMap(contentParameters.getCategoryExtendDataList(), categoryExtendList);
	...
	public String check(Long[] ids, String _csrf, HttpServletRequest request, HttpSession session, ModelMap model) {
		...
		if (CommonUtils.notEmpty(ids)) {
			...
			logOperateService
					.save(new LogOperate(site.getId(), userId, LogLoginService.CHANNEL_WEB_MANAGER, "check.content",
	...
	public String uncheck(Long[] ids, String _csrf, HttpServletRequest request, HttpSession session, ModelMap model) {
		...
		if (CommonUtils.notEmpty(ids)) {
			...
			logOperateService.save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
					LogLoginService.CHANNEL_WEB_MANAGER, "uncheck.content", RequestUtils.getIpAddress(request),
	...
	public String refresh(Long[] ids, String _csrf, HttpServletRequest request, HttpSession session, ModelMap model) {
		...
		if (CommonUtils.notEmpty(ids)) {
			SysSite site = getSite(request);
			Set<Integer> categoryIdSet = new HashSet<>();
			for (CmsContent entity : service.refresh(site.getId(), ids)) {
				categoryIdSet.add(entity.getCategoryId());
			}
	...
	public String related(CmsContentRelated entity, String _csrf, HttpServletRequest request, HttpSession session,
		...
		SysSite site = getSite(request);
		if (null != content && null != related && site.getId() == content.getSiteId()
			...
			logOperateService.save(
					new LogOperate(site.getId(), user.getId(), LogLoginService.CHANNEL_WEB_MANAGER, "related.content",
							RequestUtils.getIpAddress(request), CommonUtils.getDate(), JsonUtils.getString(related)));
		}
		return CommonConstants.TEMPLATE_DONE;
	}
	...
	public String unrelated(Long id, String _csrf, HttpServletRequest request, HttpSession session, ModelMap model) {
		...
		if (null != entity) {
			...
			if (null == content || site.getId() == content.getSiteId()) {
				...
				publish(new Long[] { entity.getContentId() }, _csrf, request, session, model);
				logOperateService.save(new LogOperate(site.getId(),
						ControllerUtils.getAdminFromSession(session).getId(), LogLoginService.CHANNEL_WEB_MANAGER,
						"unrelated.content", RequestUtils.getIpAddress(request), CommonUtils.getDate(),
	...
	public String move(Long[] ids, Integer categoryId, String _csrf, HttpServletRequest request, HttpSession session,
		...
		if (CommonUtils.notEmpty(ids) && null != category && site.getId() == category.getSiteId()) {
			...
			logOperateService.save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
					...
					CommonUtils.getDate(), new StringBuilder(StringUtils.join(ids, ',')).append(" to ")
	 ...
	 */
	@RequestMapping("sort")
	public String sort(Long id, int sort, String _csrf, HttpServletRequest request, HttpSession session,
			ModelMap model) {
		if (ControllerUtils.verifyNotEquals("_csrf", ControllerUtils.getAdminToken(request), _csrf, model)) {
			return CommonConstants.TEMPLATE_ERROR;
		}
		...
		if (CommonUtils.notEmpty(id)) {
			...
			logOperateService.save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
					...
					CommonUtils.getDate(), new StringBuilder().append(entity.getId()).append(":")
							.append(entity.getTitle()).append(" to ").append(sort).toString()));
		}
		return CommonConstants.TEMPLATE_DONE;
	}
	...
	public String delete(Long[] ids, String _csrf, HttpServletRequest request, HttpSession session, ModelMap model) {
		...
		if (CommonUtils.notEmpty(ids)) {
			Set<Integer> categoryIdSet = new HashSet<>();
			for (CmsContent entity : service.delete(site.getId(), ids)) {
				categoryIdSet.add(entity.getCategoryId());
			}
			if (!categoryIdSet.isEmpty()) {
				Integer[] categoryIds = categoryIdSet.toArray(new Integer[categoryIdSet.size()]);
	...
	public String realDelete(Long[] ids, String _csrf, HttpServletRequest request, HttpSession session,
			ModelMap model) {
		if (ControllerUtils.verifyNotEquals("_csrf", ControllerUtils.getAdminToken(request), _csrf, model)) {
			return CommonConstants.TEMPLATE_ERROR;
		}
