...
public class CmsImportAdminController {
    ...
    @ResponseBody
    public List<String> upload(@RequestAttribute SysSite site, @SessionAttribute SysUser admin, String field, MultipartFile file,
    ...
    public String reptile(@RequestAttribute SysSite site, @SessionAttribute SysUser admin, @RequestBody Map<String, String> map) {
        ...
        if (!CommonUtils.empty(sourceUrl)) {
            ...
            CmsContentAttribute cmsContentAttributeDaoByEntity = cmsContentAttributeService.findByEntity(cmsContentAttribute);
            if (cmsContentAttributeDaoByEntity != null) {
                // TODO return JsonResp.fail("文章已导入");
                return CommonConstants.TEMPLATE_ERROR;
            }
        }
        ...
        if (CommonUtils.empty(title) || CommonUtils.empty(description) || CommonUtils.empty(text)
                || CommonUtils.empty(importDateStr)) {
            // TODO "缺少部分内容title|author|description|text|importDateStr"
            return CommonConstants.TEMPLATE_ERROR;
        }
        ...
        // 插入文章
        try {
            CmsContent cmsContent = createContent(cmsCategory.getId(), IMPORT_DEFAULT_VALUE_CONTENT_MODELID, title, author,
                    editor, description, cover, site, admin);
        ...
        for (int i = 0; i < importDataList.size(); i++) {
            ...
            try {
                ...
                if (!saveTagByName(site, name)) {
                    resultList.add(MessageFormat.format("第{0}行插入时发现数据已存在，不进行操作", i + IMPORT_DEFAULT_VALUE_IGNORE_ROW_COUNT));
                }
            } catch (Exception e) {
                resultList.add(
                        MessageFormat.format("第{0}行插入时出现错误，错误原因：{1}", i + IMPORT_DEFAULT_VALUE_IGNORE_ROW_COUNT, e.getMessage()));
            }
        }
        // 保存日志
        logOperateService.save(new LogOperate(site.getId(), admin.getId(), LogLoginService.CHANNEL_WEB_MANAGER, "import.tag",
     ...
     * @return 错误列表
     */
    private List<String> importCmsCategory(SysSite site, SysUser admin, MultipartFile file, HttpServletRequest request) {
        List<String> resultList = new ArrayList<>();
        InputStream fileInputStream;
        List<List<Object>> importDataList;
        try {
        ...
        for (int i = 0; i < importDataList.size(); i++) {
            ...
            try {
                ...
                if (!saveCategory(site, admin, cmsCategory, cmsCategoryAttribute, cmsCategoryParameters, request)) {
                    resultList
                            .add(MessageFormat.format("第{0}行插入时发现数据已存在，不进行操作<br />", i + IMPORT_DEFAULT_VALUE_IGNORE_ROW_COUNT));
                }
            } catch (Exception e) {
        ...
        // 保存日志
        logOperateService.save(new LogOperate(site.getId(), admin.getId(), LogLoginService.CHANNEL_WEB_MANAGER, "import.category",
                RequestUtils.getIpAddress(request), CommonUtils.getDate(), JsonUtils.getString(importDataList)));
        IOUtils.closeQuietly(fileInputStream);
    ...
    private List<String> importCmsContent(SysSite site, SysUser admin, MultipartFile file, HttpServletRequest request) {
        ...
        for (int i = 0; i < importDataList.size(); i++) {
            ...
            try {
                ...
                CmsContent cmsContent = createContent(cmsCategoryByName.getId(), IMPORT_DEFAULT_VALUE_CONTENT_MODELID, title,
    ...
    private CmsCategoryParameters createCmsCategoryParameters(String tagTypes) {
        ...
        CmsCategoryModelParameters cmsCategoryModelParameters = createCmsCategoryModelParameters(0, "1", "/system/article.html",
                true);
        CmsCategoryModelParameters cmsCategoryModelParameters2 = createCmsCategoryModelParameters(0, "2", "", true);
        CmsCategoryModelParameters cmsCategoryModelParameters3 = createCmsCategoryModelParameters(0, "3", "/system/picture.html",
                true);
        CmsCategoryModelParameters cmsCategoryModelParameters4 = createCmsCategoryModelParameters(0, "4", "/system/book.html",
                true);
        CmsCategoryModelParameters cmsCategoryModelParameters5 = createCmsCategoryModelParameters(0, "5", "", true);
        CmsCategoryModelParameters cmsCategoryModelParameters6 = createCmsCategoryModelParameters(0, "6", "/system/chapter.html",
                true);
     ...
     */
    private CmsCategoryModelParameters createCmsCategoryModelParameters(int categoryId, String modelId, String templatePath,
            boolean use) {
