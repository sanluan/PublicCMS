...
public class CmsVoteAdminController extends AbstractController {
     ...
     */
    @RequestMapping("save")
    public String save(CmsVote entity, String _csrf, HttpServletRequest request, HttpSession session, ModelMap model) {
        ...
        if (null != entity.getId()) {
            ...
            service.save(entity);
            logOperateService.save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
                    LogLoginService.CHANNEL_WEB_MANAGER, "save.vote", RequestUtils.getIpAddress(request), CommonUtils.getDate(),
