...
public class SysSiteAdminController extends AbstractController {
    ...
    public String save(SysSite entity, String domain, Boolean wild, String roleName, String deptName, String userName,
        ...
        if (null != entity.getId()) {
            ...
            if (null != entity) {
                logOperateService
                        .save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
            ...
            SysUser user = new SysUser(entity.getId(), userName, VerificationUtils.md5Encode(password), userName,
                    dept.getId(), role.getId().toString(), null, false, true, false, null, null, 0,
                    CommonUtils.getDate());
            ...
            logOperateService.save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
                    LogLoginService.CHANNEL_WEB_MANAGER, "save.site", RequestUtils.getIpAddress(request),
                    CommonUtils.getDate(), JsonUtils.getString(entity)));
        }
        siteComponent.clear();
        if (!getSite(request).getId().equals(site.getId())
                || site.getId().equals(entity.getId()) && (!site.getSitePath().equals(entity.getSitePath())
                        || !site.getDynamicPath().equals(entity.getDynamicPath()))) {
    ...
    public String delete(Short id, String _csrf, HttpServletRequest request, HttpSession session, ModelMap model) {
        ...
        if (null != entity) {
            ...
            for (SysDomain domain : (List<SysDomain>) domainService.getPage(entity.getId(), null, null, null)
                    .getList()) {
                ...
                logOperateService.save(new LogOperate(site.getId(), userId, LogLoginService.CHANNEL_WEB_MANAGER,
                        "delete.domain", ip, now, JsonUtils.getString(entity)));
            }
            logOperateService.save(new LogOperate(site.getId(), userId, LogLoginService.CHANNEL_WEB_MANAGER,
                    "delete.site", ip, now, JsonUtils.getString(entity)));
        }
        return CommonConstants.TEMPLATE_DONE;
    }
    ...
    public String upload(MultipartFile file, String _csrf, HttpServletRequest request, HttpSession session,
            ModelMap model) {
        SysSite site = getSite(request);
        if (ControllerUtils.verifyCustom("noright", !siteComponent.isMaster(site.getId()), model)
                || ControllerUtils.verifyNotEquals("_csrf", ControllerUtils.getAdminToken(request), _csrf, model)) {
        ...
        if (null != file && !file.isEmpty()) {
            try {
                ...
                logUploadService.save(new LogUpload(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
                        LogLoginService.CHANNEL_WEB_MANAGER, "license.dat", LogUploadService.FILE_TYPE_OTHER,
                        file.getSize(), RequestUtils.getIpAddress(request), CommonUtils.getDate(),
