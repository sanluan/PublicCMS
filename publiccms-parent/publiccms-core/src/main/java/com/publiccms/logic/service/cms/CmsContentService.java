...
public class CmsContentService extends BaseService<CmsContent> {
	...
	public PageHandler query(Short siteId, String text, Long[] tagIds, Integer categoryId, Boolean containChild,
		...
		return dao.query(siteId, text, arrayToDelimitedString(tagIds, CommonConstants.BLANK_SPACE),
				getCategoryIds(containChild, categoryId, categoryIds), modelIds, startPublishDate, endPublishDate,
	...
	public FacetPageHandler facetQuery(Short siteId, String[] categoryIds, String[] modelIds, String text,
			Long[] tagIds, Date startPublishDate, Date endPublishDate, String orderField, Integer pageIndex,
			Integer pageSize) {
		return dao.facetQuery(siteId, categoryIds, modelIds, text,
				arrayToDelimitedString(tagIds, CommonConstants.BLANK_SPACE), startPublishDate, endPublishDate,
				orderField, pageIndex, pageSize);
	}
		...
		for (CmsContent entity : getEntitys(ids)) {
			if (siteId == entity.getSiteId() && !entity.isDisabled()) {
				if (0 < entity.getChilds()) {
					for (CmsContent child : (List<CmsContent>) getPage(new CmsContentQuery(siteId, null, null, null,
							null, null, entity.getId(), null, null, null, null, null, null, null, null, null), null,
	...
	public List<CmsContent> recycle(short siteId, Serializable[] ids) {
		...
		for (CmsContent entity : getEntitys(ids)) {
			if (siteId == entity.getSiteId() && entity.isDisabled()) {
				if (0 < entity.getChilds()) {
					for (CmsContent child : (List<CmsContent>) getPage(new CmsContentQuery(siteId, null, null, null,
							null, null, entity.getId(), null, null, null, null, null, null, null, null, null), false,
							null, null, null, null).getList()) {
