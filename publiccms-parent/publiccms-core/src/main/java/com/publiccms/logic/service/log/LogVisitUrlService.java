...
import com.publiccms.logic.dao.log.LogVisitUrlDao;
...
@Service
@Transactional
public class LogVisitUrlService extends BaseService<LogVisitUrl> {
    @Autowired
    ...
    public PageHandler getPage(short siteId, Date startVisitDate, Date endVisitDate, Integer pageIndex, Integer pageSize) {
        ...
        if ((null == pageIndex || 1 == pageIndex) && (null == endVisitDate || DateUtils.isSameDay(now, endVisitDate))) {
            ((List<LogVisitUrl>) page.getList()).addAll(0, logVisitService.getUrlList(siteId, now));
        }
        return page;
    }
