 ...
 * <pre>
 *  &lt;script&gt;
   fetch('${site.dynamicPath}api/directive/cms/dictionaryExcludeValue?dictionaryId=data&amp;excludeDictionaryId=data1&amp;value=1').then(res => res.json()).then(data=>{
        ...
        if (CommonUtils.notEmpty(dictionaryId) && CommonUtils.notEmpty(excludeDictionaryId)) {
            ...
            if (CommonUtils.notEmpty(value)) {
                ...
                if (CommonUtils.notEmpty(values)) {
                    CmsDictionaryExcludeValueId[] ids = Stream.of(values)
                            .map(e -> new CmsDictionaryExcludeValueId(dictionaryId, siteId, excludeDictionaryId, e))
                            .toArray(CmsDictionaryExcludeValueId[]::new);
