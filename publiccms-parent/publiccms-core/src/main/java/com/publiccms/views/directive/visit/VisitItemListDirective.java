 ...
 * <li><code>orderField</code> 排序字段,【uv:用户浏览数,ipviews:IP浏览数,pv:页面浏览数】,默认pv倒序
 ...
 * <pre>
&lt;script&gt;
fetch('${site.dynamicPath}api/directive/visit/itemList?itemType=content',{"headers":{"appToken":"接口访问授权Token"}}).then(res => res.json()).then(data=>{
...
public class VisitItemListDirective extends AbstractTemplateDirective {
    ...
    public void execute(RenderHandler handler) throws IOException, TemplateException {
        PageHandler page = service.getPage(getSite(handler).getId(), handler.getDate("startVisitDate"),
                handler.getDate("endVisitDate"), handler.getBoolean("dayAnalytics", true), handler.getString("itemType"),
                handler.getString("itemId"), handler.getString("orderField"), handler.getInteger("pageIndex", 1),
