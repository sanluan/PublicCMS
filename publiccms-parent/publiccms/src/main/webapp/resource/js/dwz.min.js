...
( function($) {
    $.extend($.fn, {
            ...
            return this.each(function() {
                $(">li", this).each(function() {
                    ...
                    $this.genTree({
                        ...
                        options: op.options, level: level, space: isLast ? null: op.space, showSub: false, isLast: isLast, async: op.options.async
                    });
                });
            });
        },
        genTree: function(options) {
            var op = $.extend({
                icon: options.icon, ckbox: options.ckbox, exp: "", coll: "", showSub: false, level: 0, options: null, isLast: false, async: options.async
            }, options);
            return this.each(function() {
                var node = $(this);
                var tree = $(">ul", node);
                ...
                if (tree.size() > 0 || node.attr(op.async)) {
                    ...
                    $(">div>div:first,>div>a", node).click(function() {
                    	if(node.attr(op.async)){
                        	$.ajax({
                                type: 'get', url: node.attr(op.async), async: false, data: {}, success: function(response){
                                	node.append(response);
                                	tree = $(">ul", node).hide();
                                	node.removeAttr(op.async);
                                },error: DWZ.ajaxError
                            });
                        }
                    	if(node.attr(op.async) ){
