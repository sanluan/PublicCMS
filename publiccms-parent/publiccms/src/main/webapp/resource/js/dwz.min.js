...
function initLink($p) {
    ...
    $("a[target=navTab]", $p).each(function() {
        $(this).click(function(event) {
            ...
            var newWindow = (navigator.platform.match("Mac") ? e.metaKey : e.ctrlKey);
            DWZ.debug(url);
            if (!url.isFinishedTm() ) {
    ...
    $("a[target=dialog]", $p).each(function() {
        $(this).click(function(event) {
            ...
            options.focusNewWindow = (navigator.platform.match("Mac") ? e.metaKey : e.ctrlKey);
            var url = $this.attr("href").replaceTmById($(event.target).parents(".unitBox:first"));
            DWZ.debug(url);
            if (!url.isFinishedTm() ) {
                alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
...
var navTab = {
    ...
    openTab: function(tabid, url, options) { // if found tabid replace tab,
        ...
        if (iOpenIndex >= 0 && !focusNewWindow) {
        ...
        itemDetail: function() {
            return this.each(function() {
                ...
                if (addButTxt ) {
                    ...
                    if(batchUploadUrl){
                        var $batchButton = $('<label><a class="btnAttach" lookupGroup="" href="'+batchUploadUrl+'" width="1000" height="600" ></a></label>').initUI().insertBefore($table).find('a');
